...
export default {
  ...
  props: {
    ...
    fieldNameOptions: {
      type: Array,
      default: () => [{ label: "a1", value: 'a1',type: 'string' }, { label: "a2", value: 'a2',type: 'number' }, { label: "a3", value: 'a3', type: 'date' }, { label: "a4", value: 'a4',type:'enum' }]
    },
    /**
     * 关系运算符下拉框的所有数据
     */
  ...
  methods: {
    // 组件内部打印当前数据
    handleLogNow(){
      console.log(this.conditionValue,"当前配置的规则结构");
      // 返回规则配置的最终结果父组件
      ...
      return (
        <div class='condition-group'>
          ...
          <div class='condition-group-controller'>
            <el-select style='width:100px;' value={type} onChange={(value)=>{
            ...
            <div>
              <el-button type='primary' style='marginRight:5px' onClick={() => { this.handleAdd({ position }) }}>添加条件</el-button>
              <el-button type='primary' onClick={() => { this.handleAdd({ position, type: 'group' }) }}>添加一组条件</el-button>
            </div>
          </div>
          {/** 内容栏 */}
          <div>
